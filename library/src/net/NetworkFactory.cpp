...
namespace net {
	...
	NetworkFactory::NetworkFactory(const std::string & listenIP, const unsigned short listenPort, const std::string & connectIP, const unsigned short connectPort, pubsub::PubSubSystemEnvironment * pssi) : _listenIP(listenIP), _listenPort(listenPort), _connectIP(connectIP), _connectPort(connectPort), _pssi(pssi), tcpcontroller_(nullptr), udpcontroller_(nullptr), clocktcpcontroller_(nullptr) {
	}
