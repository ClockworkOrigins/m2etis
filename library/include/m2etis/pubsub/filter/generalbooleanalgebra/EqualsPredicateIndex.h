...
namespace filter {
	...
	class EqualsPredicateIndex : public PredicateIndex<EventType>  {
	public:
		EqualsPredicateIndex(AttributeName attribute_id, PredicateIdentifierFactory* predicate_identifier_factory) : PredicateIndex<EventType>(attribute_id, EQUALS, predicate_identifier_factory) {}
		...
		PredicateIdentifierFactory::PredicateID addPredicate(const EqualsAttributeFilter<EventType, AttributeType> * equals_attribute_filter) {
			...
			if (predicate_id_to_attribute_value_.find(equals_attribute_filter->get_constants()[0]) != predicate_id_to_attribute_value_.end()) {
				return predicate_id_to_attribute_value_[equals_attribute_filter->get_constants()[0]];
			}
			...
			predicate_id_to_attribute_value_.insert(std::pair<AttributeType, PredicateIdentifierFactory::PredicateID>(equals_attribute_filter->get_constants()[0], new_predicate_id));
		...
		virtual void determineMatchingPredicates(const EventType & event, std::vector<bool> & fulfilled_predicate_vector) override {
			...
			if (attribute_value_iterator == attributes_accessor_map.end()) {
				M2ETIS_THROW_API("EqualsPredicateIndex function determineMatchingPredicates", "Attribute not found in attribute map.");
		 	}

			fulfilled_predicate_vector[predicate_id_to_attribute_value_[(dynamic_cast<AttributeAccessor<EventType, AttributeType> *>(attribute_value_iterator->second.get()))->getAttributeValue(event)]] = true;
		}

		virtual ~EqualsPredicateIndex() {}
