...
#ifndef __M2ETIS_PUBSUB_FILTER_GENERALBOOLEANALGEBRA_LESSTHANPREDICATEINDEX_H__
#define __M2ETIS_PUBSUB_FILTER_GENERALBOOLEANALGEBRA_LESSTHANPREDICATEINDEX_H__
...
namespace filter {
	...
	class LessThanPredicateIndex : public PredicateIndex<EventType>  {
	public:
		LessThanPredicateIndex(AttributeName attribute_id, PredicateIdentifierFactory * predicate_identifier_factory) : PredicateIndex<EventType>(attribute_id, LESSTHAN, predicate_identifier_factory) {}
		...
		PredicateIdentifierFactory::PredicateID addPredicate(const LessThanAttributeFilter<EventType, AttributeType> * less_than_attribute_filter) {
			...
			if (predicate_id_to_attribute_value_.find(less_than_attribute_filter->get_constants()[0]) != predicate_id_to_attribute_value_.end()) {
				return predicate_id_to_attribute_value_[less_than_attribute_filter->get_constants()[0]];
			}
			...
			predicate_id_to_attribute_value_.insert(std::pair<AttributeType, PredicateIdentifierFactory::PredicateID >(less_than_attribute_filter->get_constants()[0], new_predicate_id));
		...
		virtual void determineMatchingPredicates(const EventType & event, std::vector<bool> & fulfilled_predicate_vector) override {
			...
			if (attribute_value_iterator == attributes_accessor_map.end()) {
				M2ETIS_THROW_API("LessThanPredicateIndex function determineMatchingPredicates", "Attribute not found in attribute map.");
			}

			auto ithigh = predicate_id_to_attribute_value_.upper_bound((dynamic_cast<AttributeAccessor<EventType, AttributeType> *>(attribute_value_iterator->second.get()))->getAttributeValue(event));

			for (auto iter_predicates = ithigh ; iter_predicates != predicate_id_to_attribute_value_.end() ; ++iter_predicates) {
